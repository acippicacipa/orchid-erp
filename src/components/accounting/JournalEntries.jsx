  ...
  const JournalEntries = () => {
    ...
    const fetchAccounts = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/accounting/accounts/?is_active=true`, {
          headers: {
    ...
    const fetchEntries = async () => {
      try {
        setLoading(true);
        let url = `${API_BASE_URL}/accounting/journal-entries/`;
    ...
    const handlePostEntry = async (entryId) => {
      if (window.confirm('Are you sure you want to post this journal entry? This action cannot be undone.')) {
        try {
          const response = await fetch(`${API_BASE_URL}/accounting/journal-entries/${entryId}/post_entry/`, {
            method: 'POST',
            headers: {
              'Authorization': `Token ${token}`,
              'Content-Type': 'application/json',
    ...
    const handleCancelEntry = async (entryId) => {
      if (window.confirm('Are you sure you want to cancel this journal entry?')) {
        try {
          const response = await fetch(`${API_BASE_URL}/accounting/journal-entries/${entryId}/cancel_entry/`, {
            method: 'POST',
            headers: {
              'Authorization': `Token ${token}`,
              'Content-Type': 'application/json',
